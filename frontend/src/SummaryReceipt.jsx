...
export function SummaryReceipt({ form, referenceNumber, language = 'en', onBack }) {
  ...
  const maskId = (id) => (id && id.length === 14 ? `${id.slice(0, 3)}••••••••${id.slice(-3)}` : id || '—')
  const maskPhone = (phone) => (phone && phone.length === 11 ? `${phone.slice(0, 3)}••••${phone.slice(-4)}` : phone || '—')
      ...
      >
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '12px 24px', fontSize: '13px', marginBottom: '28px' }}>
          ...
          <div>
            ...
            <strong>{form.governorate ? `${form.governorate} · ${form.address || ''}` : '—'}</strong>
          </div>
          <div>
